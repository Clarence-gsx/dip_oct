...
def train(args,train_loader, model, criterion, optimizer, epoch,print_freq=10):
   # set the AverageMeter 
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        dice_score,dice_1,dice_2,dice_3,dice_4,dice_5,dice_6,dice_7,dice_8,dice_9 = compute_average_dice(pred_seg.flatten(),label_seg.flatten())
        # metric acc for cls
        #pred_cls = (output_cls > 0.5)
        #label_cls = target_var_cls.cpu().data.numpy()
        #print(target_var_cls.size(0))
        #total += target_var_cls.size(0)*10        ############# n_classes-1
        #print(pred_cls.eq(target_var_cls.byte()))
        ...
        Dice_4.update(dice_4)
        Dice_5.update(dice_5)
        Dice_6.update(dice_6)
        Dice_7.update(dice_7)
        Dice_8.update(dice_8)
        Dice_9.update(dice_9)
        # backwards
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if i % print_freq == 0:
            logger_vis.info('Epoch: [{0}][{1}/{2}]\t'
                        ...
                        'Dice_9 {dice_9.val:.6f} ({dice_9.avg:.4f})\t'.format(
                epoch, i, len(train_loader), batch_time=batch_time,dice = dice,dice_1=Dice_1,dice_2=Dice_2,dice_3=Dice_3,dice_4=Dice_4,dice_5=Dice_5,dice_6=Dice_6,dice_7=Dice_7,dice_8=Dice_8,dice_9=Dice_9))
            print('loss :',loss.cpu().data.numpy())
            #print('Acc: %.3f%% (%d/%d)'% (100.*correct/total, correct, total))

    return losses.avg,dice.avg,Dice_1.avg,Dice_2.avg,Dice_3.avg,Dice_4.avg,Dice_5.avg,Dice_6.avg,Dice_7.avg,Dice_8.avg,Dice_9.avg
...
def val_seg(args,model):
    ...
    cudnn.benchmark = True
    dice_avg,dice_1,dice_2,dice_3,dice_4,dice_5,dice_6,dice_7,dice_8,dice_9,dice_list = val(args,val_loader, model)

    return dice_avg,dice_1,dice_2,dice_3,dice_4,dice_5,dice_6,dice_7,dice_8,dice_9,dice_list
...
def main():
    ...
    result_path = osp.join('result',task_name,'train',args.name + '_' + pretrained + '_' + args.loss + '_' +str(args.lr) + '_ttt')
    if not exists(result_path):
        os.makedirs(result_path)
    resume = True if args.resume else False
    logger = Logger(osp.join(result_path,'dice_epoch.txt'), title='dice',resume=resume)
    #if not resume:
    logger.set_names(['Epoch','Dice_Train','Dice_Val','Dice_1','Dice_11','Dice_2','Dice_22','Dice_3','Dice_33','Dice_4','Dice_44','Dice_5','Dice_55','Dice_6','Dice_66','Dice_7','Dice_77','Dice_8','Dice_88','Dice_9','Dice_99',])
    # train_seg
